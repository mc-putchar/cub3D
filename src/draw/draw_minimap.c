...
/*   Updated: 2023/09/23 19:28:50 by mcutura          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	draw_player(t_mlx_image *img, int off)
{
	...
	y = -1;
	while (++y < (MINIMAP_SIZE >> MINIMAP_PDIV))
	{
		x = -1;
		while (++x < (MINIMAP_SIZE >> MINIMAP_PDIV))
			put_pixel(img, x + off, y + off, 0xAA00CCFF);
	}
}
...
void	draw_minimap(t_mlx_image *img, t_map *map, t_vector position)
{
	...
	while (++y < MINIMAP_SIZE)
	{
		...
		{
			if (offx + (x >> MINIMAP_PDIV) < 0 || \
				(t_size)(offx + (x >> MINIMAP_PDIV)) >= map->width)
				continue ;
			if (map->val[tmp][offx + (x >> MINIMAP_PDIV)] == '1')
				put_pixel(img, x, y, 0xFFFFFFFF);
			else
				put_pixel(img, x, y, 0x33333333);
		}
	}
